...
#include <map>
#include <cstdlib>
#include "com_eclipsesource_v8_V8.h"
...
class InspectorClient : public v8_inspector::V8InspectorClient {
 public:
  InspectorClient(V8Runtime * runtime, bool connect);
  v8_inspector::V8InspectorSession* GetSession();
  void runMessageLoopOnPause(int context_group_id) override;
  void runIfWaitingForDebugger(int context_group_id) override;
  void quitMessageLoopOnPause() override;
  void resetContext(Local<Context> oldContext, Local<Context> context);
 private:
  virtual Local<Context> ensureDefaultContextInGroup(int group_id) override;
...
class InspectorFrontend final : public v8_inspector::V8Inspector::Channel {
 public:
  explicit InspectorFrontend(V8Runtime * runtime, Isolate * isolate) {
    runtime_ = runtime;
    isolate_ = isolate;
  }
...
InspectorClient::InspectorClient(V8Runtime * runtime, bool connect) {
    if (!connect) return;
    isolate_ = runtime->isolate;
    runtime_ = runtime;
    channel_.reset(new InspectorFrontend(runtime, isolate_));
...
void InspectorClient::resetContext(Local<Context> oldContext, Local<Context> context) {
    // inspector_->contextDestroyed(oldContext);
...
void InspectorClient::runMessageLoopOnPause(int context_group_id) {
    JNIEnv * env;
    getJNIEnv(env);
    env->CallVoidMethod(runtime_->v8, v8runMessageLoopOnPauseMethodID);
}

void InspectorClient::quitMessageLoopOnPause() {
    JNIEnv * env;
    getJNIEnv(env);
    env->CallVoidMethod(runtime_->v8, v8quitMessageLoopOnPauseMethodID);
}

void InspectorClient::runIfWaitingForDebugger(int contextGroupId) {
    JNIEnv * env;
    getJNIEnv(env);
    env->CallVoidMethod(runtime_->v8, v8runIfWaitingForDebuggerMethodID);
}
...
JNIEXPORT void JNICALL Java_com_eclipsesource_v8_V8__1sendInspectorMessage
  (JNIEnv * env, jobject, jlong v8RuntimePtr, jstring string) {
    V8Runtime* runtime = reinterpret_cast<V8Runtime*>(v8RuntimePtr);
    Isolate::Scope isolate_scope(runtime->isolate);
    HandleScope scope(runtime->isolate);

    v8_inspector::V8InspectorSession* session = runtime->inspectorClient->GetSession();
...
JNIEXPORT void JNICALL Java_com_eclipsesource_v8_V8__1schedulePauseOnNextStatement
  (JNIEnv * env, jobject, jlong v8RuntimePtr, jstring breakReason, jstring breakDetails) {

    V8Runtime* runtime = reinterpret_cast<V8Runtime*>(v8RuntimePtr);

    v8_inspector::StringView breakReason_view;
    v8_inspector::StringView breakDetails_view;
    v8_inspector::V8InspectorSession* session = runtime->inspectorClient->GetSession();
    session->schedulePauseOnNextStatement(breakReason_view, breakDetails_view);
}


JNIEXPORT void JNICALL Java_com_eclipsesource_v8_V8__1resetContext
  (JNIEnv * env, jobject , jlong v8RuntimePtr) {
    V8Runtime* runtime = reinterpret_cast<V8Runtime*>(v8RuntimePtr);
    Isolate::Scope isolate_scope(runtime->isolate);
    HandleScope scope(runtime->isolate);

    v8::Local<v8::ObjectTemplate> global = v8::ObjectTemplate::New(runtime->isolate);
    Handle<Context> context = v8::Context::New(runtime->isolate, NULL, global);
    
    runtime->inspectorClient->resetContext(runtime->context_.Get(runtime->isolate), context);
...
jlong createRuntime(JNIEnv *env, jobject v8, jstring globalAlias, jstring snapshotBlob);

JNIEXPORT jlong JNICALL Java_com_eclipsesource_v8_V8__1createIsolateWithSnapshotBlob
 (JNIEnv *env, jobject v8, jstring globalAlias, jstring snapshotBlob) {
   return createRuntime(env, v8, globalAlias, snapshotBlob);
}

JNIEXPORT jlong JNICALL Java_com_eclipsesource_v8_V8__1createIsolate
 (JNIEnv *env, jobject v8, jstring globalAlias) {
   return createRuntime(env, v8, globalAlias, NULL);
}
...
jlong createRuntime(JNIEnv *env, jobject v8, jstring globalAlias, jstring snapshotBlob) {
  ...
  if (snapshotBlob) {
    jboolean isCopy;
    nativeString = env->GetStringUTFChars(snapshotBlob, &isCopy);
    v8::StartupData unwarmed = v8::V8::CreateSnapshotDataBlob(nativeString);
    v8::StartupData startupData = v8::V8::WarmUpSnapshotDataBlob(unwarmed, nativeString);
    runtime->unwarmed = unwarmed;
    runtime->startupData = startupData;
    // runtime->startupData = v8::V8::WarmUpSnapshotDataBlob(runtime->unwarmed, "\nReactDOMServer.renderToStaticMarkup(React.createElement('div'));\n");
    create_params.snapshot_blob = &runtime->startupData;
    // env->ReleaseStringUTFChars(snapshotBlob, nativeString);
  }
  ...
  if (globalAlias == NULL) {
    ...
    runtime->inspectorClient = new InspectorClient(runtime, true);
