...
class InspectorClient : public v8_inspector::V8InspectorClient {
 public:
  InspectorClient(V8Runtime * runtime, Local<Context> context, bool connect);
...
class InspectorFrontend final : public v8_inspector::V8Inspector::Channel {
 public:
  explicit InspectorFrontend(V8Runtime * runtime, Local<Context> context) {
    runtime_ = runtime;
...
InspectorClient::InspectorClient(V8Runtime * runtime, Local<Context> context, bool connect) {
    if (!connect) return;
    isolate_ = context->GetIsolate();
    runtime_ = runtime;
    channel_.reset(new InspectorFrontend(runtime, context));
...
JNIEXPORT void JNICALL Java_com_eclipsesource_v8_V8__1sendInspectorMessage
  (JNIEnv * env, jobject, jlong v8RuntimePtr, jstring string) {
    ...
    v8_inspector::V8InspectorSession* session = InspectorClient::GetSession(runtime->context_.Get(runtime->isolate));
...
JNIEXPORT void JNICALL Java_com_eclipsesource_v8_V8__1schedulePauseOnNextStatement
  (JNIEnv * env, jobject, jlong v8RuntimePtr, jstring breakReason, jstring breakDetails) {
    ...
    // Local<String> v8Message = createV8String(env, runtime->isolate, breakReason);
    // int length = v8Message->Length();
    // std::unique_ptr<uint16_t[]> buffer(new uint16_t[length]);
    // v8Message->Write(buffer.get(), 0, length);
    // v8_inspector::StringView breakReason_view(buffer.get(), length);
    
    // Local<String> v8Message2 = createV8String(env, runtime->isolate, breakDetails);
    // length = v8Message2->Length();
    // std::unique_ptr<uint16_t[]> buffer2(new uint16_t[length]);
    // v8Message->Write(buffer2.get(), 0, length);
    // v8_inspector::StringView breakDetails_view(buffer.get(), length);
...
JNIEXPORT jlong JNICALL Java_com_eclipsesource_v8_V8__1createIsolate
 (JNIEnv *env, jobject v8, jstring globalAlias) {
  ...
  if (globalAlias == NULL) {
    ...
    runtime->inspectorClient = new InspectorClient(runtime, context, true);
